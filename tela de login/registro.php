...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["password"]))) {
        $password_err = "Por favor preencha uma senha";
    ...
    if (empty(trim($_POST["confirm_password"]))) {
        $password_confirm_err = "Por favor confirme a senha cavalo";
    } else {
    ...
    if (empty($username_err) && empty($password_err) && empty($password_confirm_err)) {
        ...
        if ($stmt = mysqli_prepare($connection, $sql )) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                echo "Oops! Algo de errado não esta certo. Tente novamente.";
            }
        ...
        <div class="justify-content-center align-items-center row">
            <div class="wrapper">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <p>Already have an account? <a href="teladelogin.php">Login here</a>.</p>
                </form>
            </div>
        </div>
    </div>
